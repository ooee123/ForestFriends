...
class GUI(tk.Tk):
   ...
   def __init__(self, *args, **kwargs):
      ...
      for F in (Start, ImportFile, badFileSyntax, VerifyDesign, Redesign, VerifyParts, Reverify, Machine, Finish):
...
class Machine(tk.Frame):
   ...
   def __init__(self, parent, controller):
      ...
      button1 = tk.Button(self, text="Start Machining", font=LARGE_FONT, height= 5,
         command=self.loadCoordPars)
      button1.pack(anchor=S, fill=X)

      button2 = tk.Button(self, text="Cancel", font=LARGE_FONT,height = 5,
         command=restartFrame)
         #`command=lambda: controller.show_frame(Start))
